...
const handleSelectTags = (itrem) =>{
    console.log(itrem,'handleChangePhone')
    selectTag = itrem.key
}
</script>
    ...
    <div class="questions-wp">
        ...
        <div class="tag-des-wp">
          ...
          <div class="tags-wp">
            {#each tags as itrem}
                <span
                    on:click={handleSelectTags(itrem)}
                    class={itrem.key === selectTag ? 'selected-tags tags' : 'tags'}
                    >{`${itrem.value}--${itrem.key}`}</span>
                {:else}
                <div>暂无</div>
            {/each}
          </div>
